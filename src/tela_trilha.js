...
class TelaTrilha extends Phaser.Scene {
    ...
    preload() {
        this.load.image('fundo_mapa', 'assets/identidade_de_marca/tela.trilha.jpg'); // Imagem de fundo do mapa
    ...
    create() {
        ...
        // No futuro pode vir de um banco de dados ou save local
        this.nivelDesbloqueado = this.registry.get('nivelDesbloqueado_iniciante') || 0;
    ...
    exibirPopupIlha(titulo, descricao, index) {
        ...
        if (btnContinuar) {
            btnContinuar.on('pointerdown', () => {
                fecharPopup(() => {
                    // 1. Verifica e toca o som (Isolado no seu próprio IF)
                    let permissaoSom = this.registry.get('sfx_ligado');
                    if (permissaoSom !== false) {
                        this.sound.play('clique');
                    }
                    
                    // 2. Prepara as variáveis e muda de cena (Fora do IF do som)
